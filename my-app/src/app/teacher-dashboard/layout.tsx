...
useEffect(() => {
  const fetchData = async () => {
    try {
      const userRes = await
        fetch("http://localhost:5007/teacher/me", {
          method: "GET",
          credentials: "include",
          headers: {
  ...
  const fetchData = async () => {
    try {
      const statsRes = await
        fetch("http://localhost:5007/teacher/stats", {
          method: "GET",
          credentials: "include",
          headers: {
            "Content-Type": "application/json",
      ...
      <SidebarInset className="overflow-hidden px-4 md:px-6 lg:px-8">
        <header className="flex h-16 shrink-0 items-center gap-2 border-b">
          ...
          <div className="flex gap-3 ml-auto">
            <FeedbackDialog />
            <UserDropdown firstName={user?.firstName} lastName={user?.lastName} email={user?.email} />
          </div>
        </header>
        <div className="flex flex-col gap-4 lg:gap-6 py-4 lg:py-6">
          <div className="flex items-center justify-between gap-4">
            <div className="space-y-1">
              <h1 className="text-2xl font-semibold">Hello, {user?.firstName + " " + user?.lastName}</h1>
              <p className="text-sm text-muted-foreground">
                Here's an overview of your students. Manage them all with one
                click!
              </p>
            </div>
          </div>
