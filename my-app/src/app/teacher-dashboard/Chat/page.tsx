...
export default function Chat() {
    ...
    async function fetchConversations() {
      try {
        const res = await fetch(
          "http://localhost:5007/chat/conversations/teacher",
          {
            credentials: "include",
          }
        );
  ...
  useEffect(() => {
    ...
    async function fetchMessages() {
      try {
        const res = await fetch(
          `http://localhost:5007/chat/messages/${selectedConversation}`,
          {
            credentials: "include",
          }
        );
    ...
    <div className="flex h-screen items-center bg-accent rounded-xl bg-gradient-to-br from-sidebar/60 to-sidebar border mb-5">
      <Dialog>
        <ResizablePanelGroup
          ...
          className="max-w-[1200px] p-10"
        >
          <ResizablePanel
            defaultSize={25}
            className="rounded-l-xl border-y border-l bg-white"
          >
            <ScrollArea className="h-full w-full">
              ...
              {conversations.map((conv) => {
                const initials =
                  `${conv.student.firstName[0]}${conv.student.lastName[0]}`.toUpperCase();
                ...
                return (
                  <div
                    key={conv._id}
                    className="flex items-center gap-4 p-4 border-b border-gray-200 hover:bg-gray-50 transition rounded-md cursor-pointer"
          ...
          <ResizablePanel
            defaultSize={75}
            className="rounded-r-xl border-y border-r bg-white"
          >
            <div className="h-full flex flex-col">
              ...
              <div className="flex gap-2 p-5 border-t items-center [@media(max-width:530px)]:flex-col [@media(max-width:420px)]:items-center">
                ...
                <div className="flex gap-2 overflow-x-auto scroll-container">
                  ...
                  {showEmojiPicker && (
                    <div className="absolute top-110 right-27 z-10">
                      <Picker
                        data={data}
                        theme="light"
                  ...
                  <Button
                    size="icon"
                    className="bg-blue-500 hover:bg-blue-600 border cursor-pointer "
